...
def plda_run(datatable, dicttable, modeltable, numiter, numtopics, alpha, eta, restart):
    """
    ...
    for i in range(0,numrounds):
        ...
        plpy.execute("select madlib.plda(" + str(numtopics) + "," + str(stepperround) +"," + str(restartstep + i*stepperround) + "," + str(alpha) + "," + str(eta) + ", 'localWordTopicCount', '" + modeltable + "')")
        plpy.info( 'Finished iteration %d' % (restartstep + (i+1)*stepperround))
        plpy.execute("VACUUM madlib.lda_corpus")

    if leftover > 0:
        plpy.execute("select madlib.plda(" + str(numtopics) + "," + str(leftover) + "," + str(restartstep + numrounds*stepperround) + "," + str(alpha) + "," + str(eta) + ", 'localWordTopicCount', '" + modeltable + "')")
